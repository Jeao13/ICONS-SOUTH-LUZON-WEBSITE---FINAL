...
async function getStartup(id) {
    try {
        const response = await fetch(`http://localhost:3000/api/v1/startup-info/${id}`, {
            method: 'GET'
        });
...
async function editStartup() {
    ...
    await fetch(`http://localhost:3000/api/v1/startup-info/${id}`, {
        method: 'PATCH',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
...
async function getHome(id) {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/v1/home-content/${id}`, {
            method: 'GET'
        });
...
async function getConvo(id) {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/v1/conversation/${id}`, {
            method: 'GET'
        });
    ...
    reader.onload = async function(event) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const errorMessage = await response.text();
                throw new Error(errorMessage || 'Failed to update conversation content');
            }
        } catch (error) {
